/*
 * Algorithm_to_deploy_psf.c
 *
 * Automatically generated s-function with I/O interface for:
 * Component: Algorithm_to_deploy
 * Component Simulink Path: Algorithm_to_deploy
 * Simulation Mode: PIL
 *
 */

#define S_FUNCTION_NAME                Algorithm_to_deploy_psf
...
static RegMdlInfo rtMdlInfo_Algorithm_to_deploy[1] = { "",
...
static boolean_T startAndSetupApplication(SimStruct *S)
{
...

  {
    ...
    {
      mxArray * codeGenComponent = mxCreateString("Algorithm_to_deploy");
      mxArray *rhs[3];
      char * simulinkBlockPath = getSimulinkBlockPath(S);
      if (simulinkBlockPath == NULL) {
        ssSetErrorStatus(S,
...
static void mdlInitializeSizes(SimStruct *S)
{
  ...
  slmrInitializeIOPortDataVectors(S, 12, 1);
  if (!ssSetNumInputPorts(S, 12))
    return;
  if (!ssSetInputPortMatrixDimensions(S, 0, 3, 1))
    return;
  ...
  if (!ssSetInputPortMatrixDimensions(S, 1, 3, 1))
    return;
  ...
  if (!ssSetInputPortMatrixDimensions(S, 2, 3, 1))
    return;
  ...
  if (!ssSetInputPortMatrixDimensions(S, 3, 3, 1))
    return;
  ...
  if (!ssSetInputPortMatrixDimensions(S, 4, 3, 3))
    return;
  ...
  if (!ssSetInputPortMatrixDimensions(S, 5, 3, 1))
    return;
  ...
  if (!ssSetInputPortMatrixDimensions(S, 6, 3, 1))
    return;
  ...
  if (!ssSetInputPortMatrixDimensions(S, 7, 3, 1))
    return;
  ...
  if (!ssSetInputPortMatrixDimensions(S, 8, 3, 1))
    return;
  ...
  if (S->mdlInfo->genericFcn != (NULL) &&
    ...
    ssRegModelRefMdlInfo(S, (char *)"Algorithm_to_deploy",
                         rtMdlInfo_Algorithm_to_deploy, 1);
  }
...
{
  {
    ...
    if (IOBufferPtr != NULL) {
      void * pXILService = (void *) ssGetPWorkValue(S, 9);
      if (pXILService != NULL) {
        if (xilHostAppSvcGrowIOBuffer(pXILService, IOBufferPtr, 129, 0)!=
            XILHOSTAPPSVC_SUCCESS) {
          return;
        }                              /* if */
